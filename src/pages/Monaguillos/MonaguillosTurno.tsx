...
import { getDataTurnoLector, getDataTurnoMonaguillos, TurnoType } from '../../interfaces/Turno'
    ...
    useEffect(() => {
     ...
     const getTurno = async():Promise<void> => {
        ...
        const insertActivity = async():Promise<void> => {
           let result=await Data.InsertNew(getDataTurnoMonaguillos(nombre,lugar,fecha,hora,encargado,estado),TABLENAME);
           if(result){
              Alert.SuccessInsert();
              clearInputs()
              await getTurno()
        ...
        const updateActivity =async ():Promise<void> => {
          let result=await Data.UpdateItem(getDataTurnoMonaguillos(nombre,lugar,fecha,hora,encargado,estado),TABLENAME,id)
          if(result){
            Alert.SuccessUpdate();
            clearInputs()
            await getTurno()
        ...
        const openEditModal = (params:TurnoType) => {    
          ...
          setEncargado(params.monaguillos!)
          setAction("update")
          setShowModal(true)     
      ...
      return (
       ...
       <SubDataContainer>
        {turnoData.length>0 ?
         ...
         <CardTurno item={item} key={index} type="monaguillos">
