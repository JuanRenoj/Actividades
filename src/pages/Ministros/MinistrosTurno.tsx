...
import {  getDataTurnoMinistros, TurnoType } from '../../interfaces/Turno'
    ...
    useEffect(() => {
     ...
     const getTurno = async():Promise<void> => {
        ...
        const insertActivity = async():Promise<void> => {
           let result=await Data.InsertNew(getDataTurnoMinistros(nombre,lugar,fecha,hora,encargado,estado),TABLENAME);
           if(result){
              Alert.SuccessInsert();
              clearInputs()
              await getTurno()
        ...
        const updateActivity =async ():Promise<void> => {
          let result=await Data.UpdateItem(getDataTurnoMinistros(nombre,lugar,fecha,hora,encargado,estado),TABLENAME,id)
          if(result){
            Alert.SuccessUpdate();
            clearInputs()
            await getTurno()
      ...
      return (
       ...
       <SubDataContainer>
        {turnoData.length>0 ?
         ...
         <CardTurno item={item} key={index} type="ministros">
