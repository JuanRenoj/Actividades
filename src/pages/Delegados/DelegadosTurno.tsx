...
import { getDataTurnoDelegados, TurnoType } from '../../interfaces/Turno'
...
import { Actividades, NameDelegados } from '../../util/Strings'


function DelegadosTurno() {
    ...
    useEffect(() => {
     ...
     const getTurno = async():Promise<void> => {
        ...
        const insertActivity = async():Promise<void> => {
           let result=await Data.InsertNew(getDataTurnoDelegados(nombre,lugar,fecha,hora,encargado,estado),TABLENAME);
           if(result){
              Alert.SuccessInsert();
              clearInputs()
              await getTurno()
        ...
        const updateActivity =async ():Promise<void> => {
          let result=await Data.UpdateItem(getDataTurnoDelegados(nombre,lugar,fecha,hora,encargado,estado),TABLENAME,id)
          if(result){
            Alert.SuccessUpdate();
            clearInputs()
            await getTurno()
      ...
      return (
        ...
        <Modal show={showModal} setShow={setShowModal} onSubmit={handelSubmit} title={title} >
         {/*    <TextInput value={nombre} onChange={handleNombre} label='Actividad' type='text' required/> */}
            ...
            <InputSelect item={NameDelegados} onClick={selectEncargado} onChange={handleEncargado} value={encargado} label='Seleccionar o ingresar encargado' required />  
       ...
       <SubDataContainer>
        {turnoData.length>0 ?
         ...
         <CardTurno item={item} key={index} type="delegado">
